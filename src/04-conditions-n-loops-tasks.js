...
function isCreditCardNumber(ccn) {
  let sum = 0;
  let num = 0;
  const reverse = ccn.toString().split('').reverse().join('');
  for (let i = 0; i < reverse.length; i += 1) {
    if (i % 2 !== 0 && i !== 0) {
      num = parseInt(reverse[i], 10) * 2;
      if (num > 9) {
        num -= 9;
        sum += num;
      } else {
...
function getDigitalRoot(num) {
  ...
  if (sum > 9) {
    let newSum = 0;
    for (let n = 0; n < sum.toString().length; n += 1) {
      newSum += +sum.toString()[n];
    }
    return newSum;
  }
  return sum;
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    const a = open.indexOf(str[i]);
    const b = close.indexOf(str[i + 1]);
    if (open.includes(str[i]) && close.includes(str[i + 1]) && a !== b) {
      return false;
    }
    if (open.includes(str[i])) {
      count[open.indexOf(str[i])] += 1;
    } else {
      count[close.indexOf(str[i])] -= 1;
    }
    if (count.includes(-1)) {
      return false;
    }
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 1; i < pathes.length; i += 1) {
    if (path === '') {
      for (let n = 0; n < pathes[i].length; n += 1) {
        if (pathes[i][n] === pathes[i - 1][n]) {
          path += pathes[i - 1][n];
        } else {
          break;
        }
    ...
    } else if (!pathes[i].includes(path)) {
      return '';
    }
  }
  return path;
